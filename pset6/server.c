...
char* indexes(const char* path)
{
    ...
    int n=strlen(path);
    int i=strlen(html);
    char *index=(char*)malloc((n+i+1)*sizeof(char));
    strcpy(index, path);
    strcat(index, html);
    if (access(index, 0)==0)
    {
...
{
    size_t i=0;
    int realltab=SIZETAB;
    BYTE *tab=(BYTE*)malloc(realltab*sizeof(BYTE));
    ...
    while (true)
    {
        fileof=fread(&c, 1, 1, file);
        if (i==realltab)
        {
            realltab+=SIZETAB;
            tab=(BYTE *)realloc(tab, realltab);
        }
        if (fileof==0)
            break;
        *(tab+i)=c;
...
const char* lookup(const char* path)
{
    const char *retmime=NULL;
    const char *mime=path;
    int n=strlen(path);
    for (mime=mime+(n-1); *mime!='.'; mime--)
    ;
...
char* urldecode(const char* s)
{
    ...
    /*for (c=fgetc(file); c!=EOF; c=fgetc(file))
    {
        ...
        {
            realltab+=SIZETAB;
            tab2=(BYTE *)realloc(tab2, realltab);
        }
        *tab=c;
        tab++;
        i++;
